...
# Setting up DROP polict by default
...
$IPT -A INPUT -d $LOCAL_NET -p tcp -m tcp --dport $UPS -j ACCEPT 
...
$IPT -A INPUT -s $LOCAL_NET -d $LOCAL_NET -p tcp -m tcp --sport $WINBIND -j ACCEPT 
...
$IPT -A OUTPUT -s $LOCAL_NET -p tcp -m tcp --sport $UPS -j ACCEPT 
